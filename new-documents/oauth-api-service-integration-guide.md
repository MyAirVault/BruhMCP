...
This comprehensive guide documents the current authentication architecture and provides detailed instructions for adding new OAuth and API key-based services to the MiniMCP platform. The analysis covers both Google Drive (OAuth) and Splitwise (API key) integration scenarios.
...
The MiniMCP platform implements a dual authentication system:
...
- core/oauth-manager.js       // OAuth flow orchestration
- core/token-exchange.js      // Token refresh operations
- providers/base-oauth.js     // Abstract OAuth provider base class
...
class SplitwiseMCPServer {
  ...
  async createExpense(description, cost, users) {
    const response = await axios.post(`${this.baseURL}/create_expense`, {
      description,
      cost,
      users
    }, {
...
The MiniMCP platform provides a robust, secure, and extensible architecture for both OAuth and API key-based service integration. The current implementation supports:
