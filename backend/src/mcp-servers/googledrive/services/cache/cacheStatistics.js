...
export function getCacheStatistics() {
	...
	if (stats.last_accessed_times.length > 0) {
		const avgLastAccessed = stats.last_accessed_times.reduce((a, b) => a + b, 0) / stats.last_accessed_times.length;
	...
	if (stats.refresh_attempt_counts.length > 0) {
		...
		stats.avg_refresh_attempts = stats.refresh_attempt_counts.reduce((a, b) => a + b, 0) / stats.refresh_attempt_counts.length;
	}
