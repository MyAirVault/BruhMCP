 ...
 */
function formatFileListResponse(response, query = '') {
	...
	const formattedFiles = response.files
		.map(file => formatFileResponse(file))
		.filter(/** @param {FormattedFile|null} file */ /** @returns {file is FormattedFile} */ file => file !== null);
 ...
 */
function formatUploadResponse(response, originalPath) {
 ...
 */
function formatDriveInfoResponse(about) {
	...
	return {
		user:
			about.user && typeof about.user === 'object'
				? {
 ...
 */
function formatFilePermissions(permissions) {
	...
	const summary = {
		...
		hasExternalUsers: permissions.some(
			p => p.type === 'user' && p.emailAddress && !p.emailAddress.includes('@gmail.com')
		),
	};
 ...
 */
function formatUploadResult(uploadResponse, originalPath) {
 ...
 */
function formatFolderTree(files) {
