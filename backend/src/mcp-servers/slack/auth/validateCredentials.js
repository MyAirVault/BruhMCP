/**
 * @fileoverview Slack Credential Validation
...
/**
 * Validates Slack OAuth credentials format
...
async function validateCredentials(credentials, userId) {
	try {
		console.log(`🔍 Validating Slack credentials for user: ${userId}`);
		...
		const validator = createSlackValidator(slackCredentials);

		// Validate credentials format (OAuth only does format validation)
		const result = await validator.validateFormat(slackCredentials);

		// Convert validator result to our expected format
		console.log('🔄 Slack validator result:', { valid: result.valid, error: result.error });
		
		if (result.valid) {
			const response = {
				success: true,
				message: 'Slack OAuth credentials format is valid',
				data: {
			...
			const response = {
				success: false,
				message: result.error || 'Invalid Slack OAuth credentials format'
			};
			console.log('❌ Returning failure response:', response);
			return response;
		}
	} catch (error) {
		console.error('Slack credential validation error:', error);
		return {
			success: false,
