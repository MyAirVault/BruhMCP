...
const { updateOAuthStatus, updateOAuthStatusWithLocking  } = require('../../../db/queries/mcpInstances/index');
...
async function processSuccessfulRefresh(instanceId, refreshResult, instance, req) {
  ...
  const { accessToken, refreshToken, expiresIn, scope, teamId, method } = refreshResult;
  ...
  cacheNewTokens(instanceId, accessToken, refreshToken, newExpiresAt.getTime(), instance.user_id, teamId);
  ...
  await setupRequestWithNewTokens(req, accessToken, instanceId, instance.user_id, teamId);
