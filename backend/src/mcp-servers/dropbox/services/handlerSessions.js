/**
 * Handler session management for Dropbox MCP JSON-RPC handlers
...
const { DropboxMCPHandler } = require('../endpoints/mcpHandler.js');
...
function getOrCreateHandler(instanceId, serviceConfig, bearerToken) {
	...
	if (!session) {
		...
		console.log(`🔧 Creating new Dropbox handler session for instance: ${instanceId}`);
		const handler = new DropboxMCPHandler(serviceConfig, bearerToken);
		...
		console.log(`✅ Dropbox handler session created. Total sessions: ${handlerSessions.size}`);
	} else {
		console.log(`♻️  Reusing existing Dropbox handler session for instance: ${instanceId}`);
...
function removeHandlerSession(instanceId) {
	...
	if (removed) {
		console.log(`🗑️  Removed Dropbox handler session for instance: ${instanceId}`);
	}
	return removed;
}
...
function cleanupExpiredSessions() {
	...
	for (const [instanceId, session] of handlerSessions) {
		...
		if (idleTime > SESSION_TIMEOUT) {
			...
			removedCount++;
			console.log(`🧹 Cleaned up expired Dropbox session: ${instanceId} (idle for ${Math.floor(idleTime / 60000)} minutes)`);
		}
	}
	
	if (removedCount > 0) {
		console.log(`🧹 Dropbox session cleanup complete. Removed ${removedCount} expired sessions. Active sessions: ${handlerSessions.size}`);
	}
}
...
function startSessionCleanup() {
	if (cleanupInterval) {
		console.warn('⚠️  Dropbox session cleanup already running');
		return;
	}
	
	cleanupInterval = setInterval(cleanupExpiredSessions, CLEANUP_INTERVAL);
	console.log('🧹 Started Dropbox handler session cleanup service');
}
...
function stopSessionCleanup() {
	if (cleanupInterval) {
		...
		console.log('🛑 Stopped Dropbox handler session cleanup service');
	}
	
	// Clear all sessions on shutdown
	handlerSessions.clear();
	console.log('🗑️  Cleared all Dropbox handler sessions');
}
...
function invalidateHandlerSession(instanceId) {
	...
	if (removed) {
		console.log(`🔄 Dropbox handler session invalidated due to credential change: ${instanceId}`);
	}
}
...
function updateSessionBearerToken(instanceId, newBearerToken) {
	...
	if (session && session.handler) {
		...
		session.lastAccessed = Date.now();
		console.log(`🔄 Updated bearer token in Dropbox session: ${instanceId}`);
		return true;
	}
	return false;
}
