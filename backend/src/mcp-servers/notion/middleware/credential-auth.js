...
} from '../services/credential-cache.js';
...
export function createCredentialAuthMiddleware() {
	return async (req, res, next) => {
		...
		try {
			...
			const instance = await lookupInstanceCredentials(instanceId, 'notion');
			...
			if (!instance.service_active) {
				return ErrorResponses.serviceUnavailable(res, 'Notion service is currently disabled', {
					instanceId,
					service: 'notion',
				});
			}
			...
			if (refreshToken && instance.mcp_service_name !== 'notion') {
				...
				try {
					...
					// Try OAuth service first, then fallback to direct Notion OAuth
					try {
						...
						console.log(
							`⚠️  OAuth service failed, trying direct Notion OAuth: ${oauthServiceError.message}`
						);
