/**
 * Credential watcher service for GitHub MCP OAuth token management
...
export function startCredentialWatcher() {
  if (watcherInterval) {
    console.warn('⚠️  GitHub credential watcher already running');
    return;
  }

  watcherInterval = setInterval(runCredentialWatcher, WATCHER_INTERVAL);
  watcherStats.isRunning = true;
  console.log('🔍 Started GitHub OAuth credential watcher service');
}
...
export function stopCredentialWatcher() {
  if (watcherInterval) {
    ...
    console.log('🛑 Stopped GitHub OAuth credential watcher service');
  }
}
...
async function runCredentialWatcher() {
  ...
  console.log(`🔍 Running GitHub OAuth credential watcher cycle #${watcherStats.totalRuns}`);
  ...
  try {
    ...
    console.log(`✅ GitHub credential watcher cycle completed in ${duration}ms`);

  } catch (error) {
    console.error('❌ GitHub credential watcher cycle failed:', error);
  }
}
