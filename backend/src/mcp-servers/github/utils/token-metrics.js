/**
 * Token Refresh Metrics System for GitHub
...
class TokenMetrics {
  ...
  recordRefreshSuccess(instanceId, method, startTime, endTime) {
    ...
    console.log(`📊 GitHub token refresh success: ${instanceId} via ${method} (${latency}ms)`);
  }
  ...
  recordRefreshFailure(instanceId, method, errorType, errorMessage, startTime, endTime) {
    ...
    console.log(`📊 GitHub token refresh failure: ${instanceId} via ${method} - ${errorType} (${latency}ms)`);
  }
  ...
  reset() {
    ...
    console.log('📊 GitHub token metrics reset');
  }
