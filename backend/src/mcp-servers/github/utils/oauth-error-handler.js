/**
 * OAuth error handling utilities for GitHub service
...
export function parseOAuthError(error) {
  ...
  if (message.includes('invalid_grant') || 
    ...
    return {
      ...
      requiresReauth: true,
      userMessage: 'Your GitHub authorization has expired. Please re-authenticate.',
      shouldRetry: false,
      logLevel: 'warn'
    };
  }
  ...
  if (message.includes('invalid_client') || 
    ...
    return {
      ...
      requiresReauth: true,
      userMessage: 'Invalid GitHub OAuth credentials. Please contact support.',
      shouldRetry: false,
      logLevel: 'error'
    };
  }
  ...
  if (message.includes('invalid_request') || 
    ...
    return {
      ...
      userMessage: 'GitHub OAuth request format error. Please try again.',
      shouldRetry: true,
      logLevel: 'warn'
    };
  }
  ...
  return {
    ...
    requiresReauth: false,
    userMessage: 'GitHub authentication error. Please try again.',
    shouldRetry: false,
    logLevel: 'error'
  };
}
...
export async function handleTokenRefreshFailure(instanceId, error, updateOAuthStatus) {
  ...
  console.log(`🔍 GitHub token refresh error analysis for ${instanceId}:`, {
...
export function logOAuthError(error, context, instanceId) {
  ...
  const logMessage = `GitHub OAuth error in ${context} for instance ${instanceId}: ${error.message}`;
