/**
 * OAuth Integration Utilities for Google Sheets Service
 ...
 */
async function refreshWithOAuthService(params) {
  const { refreshToken, clientId, clientSecret } = params;

  console.log(`🔄 Refreshing tokens via OAuth service with circuit breaker`);
  ...
  try {
    ...
    console.log(`✅ Tokens refreshed successfully via OAuth service with circuit breaker`);
 ...
 */
async function exchangeTokens(params) {
 ...
 */
async function validateOAuthCredentials(params) {
 ...
 */
async function isOAuthServiceAvailable() {
  try {
    const oauthServiceUrl = process.env.OAUTH_SERVICE_URL || 'http://localhost:3001';
