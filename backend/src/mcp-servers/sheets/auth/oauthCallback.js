/**
 * @fileoverview Google Sheets OAuth Callback Handler
 * Standardized function for handling Google Sheets OAuth callbacks
 */
...
/**
 * Handles OAuth callback for Google Sheets service
...
async function oauthCallback(code, state) {
	try {
		console.log(`🔄 Processing Google Sheets OAuth callback`);
		...
		console.log(`✅ Google Sheets OAuth callback completed for instance: ${instanceId}`);

		return {
			success: true,
			message: 'Google Sheets OAuth completed successfully',
			data: {
				instanceId,
