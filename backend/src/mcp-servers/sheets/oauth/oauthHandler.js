/**
 * @fileoverview Google Sheets OAuth Handler
...
class SheetsOAuthHandler {
	...
	async initiateFlow(instanceId, userId, credentials) {
		try {
			...
			console.log(`🔐 Generated Google Sheets OAuth URL for instance ${instanceId}`);
			...
			console.error('Failed to initiate Sheets OAuth flow:', error);
	...
	async handleCallback(code, state) {
		try {
			...
			console.log(`✅ Google Sheets OAuth callback successful for instance ${instanceId}`);
	...
	async refreshToken(refreshToken, credentials) {
		try {
			...
			return {
				...
				expires_in: newCredentials.expiry_date ? Math.floor((newCredentials.expiry_date - Date.now()) / 1000) : 3600,
			};
		} catch (error) {
			console.error('Failed to refresh Sheets OAuth token:', error);
