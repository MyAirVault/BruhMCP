/**
 * Credential validation utilities for Google Sheets MCP service
...
export async function validateCredentials(credentials) {
  ...
  // Validate credentials with OAuth service
  try {
    const oauthValidation = await validateOAuthCredentials({
      clientId,
      clientSecret
    });
    ...
    if (!oauthValidation.valid) {
      return {
        valid: false,
        error: 'OAuth credential validation failed',
        details: {
          oauthError: oauthValidation.error
        }
      };
    }
    ...
    return {
      valid: false,
      error: 'OAuth service validation failed',
      details: {
...
export function validateScopes(scopes) {
  ...
  const hasRequiredScopes = requiredScopes.every(scope => 
    scopes.includes(scope) || scopes.includes('https://www.googleapis.com/auth/spreadsheets')
  );
...
export function validateInstanceConfig(config) {
  ...
  if (serviceName !== 'sheets') {
    errors.push('Invalid service name (expected "sheets")');
  }
...
class GoogleSheetsOAuthValidator extends BaseValidator {
  ...
  async testCredentials(credentials) {
    ...
    try {
      ...
      return createValidationResult(false, `Failed to test Google Sheets OAuth credentials: ${error.message}`, 'credentials');
    }
  }
 ...
 */
function createGoogleSheetsValidator(credentials) {
