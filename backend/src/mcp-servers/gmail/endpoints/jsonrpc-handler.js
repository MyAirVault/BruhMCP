/**
 * Gmail MCP JSON-RPC protocol handler
 * Implements proper JSON-RPC 2.0 message handling for Gmail MCP server
 */

import { executeToolCall } from './call.js';
import { getTools } from './tools.js';

export class GmailMCPJsonRpcHandler {
	...
	async handleInitialize(params, id) {
		...
		return this.createSuccessResponse(id, {
			...
			instructions: `This is a ${this.serviceConfig.displayName} MCP server providing tools for Gmail API integration. Available tools include: send_email, fetch_emails, reply_to_email, create_draft, manage_labels, and more. Authentication uses OAuth 2.0 Bearer tokens with scopes: ${this.serviceConfig.scopes.join(', ')}`,
		});
	}
	...
	async handleToolsList(params, id) {
		...
		return this.createSuccessResponse(id, {
			...
			// No pagination needed for Gmail tools, but include nextCursor if provided
	...
	async handleResourcesList(params, id) {
		...
		// Gmail server doesn't provide resources, return empty list
		return this.createSuccessResponse(id, {
			resources: [],
		});
	}
	...
	async handleResourcesRead(params, id) {
		...
		// Gmail server doesn't provide resources
