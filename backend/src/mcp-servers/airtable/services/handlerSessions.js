/**
 * Handler session management for Airtable MCP JSON-RPC handlers
...
const { AirtableMCPHandler } = require('../endpoints/mcpHandler.js');
 ...
 */
function getOrCreateHandler(instanceId, serviceConfig, apiKey) {
	...
	if (!session) {
		...
		const handler = new AirtableMCPHandler(serviceConfig, apiKey);
