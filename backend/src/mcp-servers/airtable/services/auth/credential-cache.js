/**
 * Credential cache service for Airtable MCP instance management
 * Phase 2: Token Management and Caching System implementation
 * 
 * This service manages in-memory caching of Airtable API keys
 * to reduce database hits and improve request performance.
 */

// Global credential cache for Airtable service instances
const airtableCredentialCache = new Map();
...
export function getCachedCredential(instanceId) {
	...
	if (cached.expires_at && new Date(cached.expires_at) < new Date()) {
		...
		airtableCredentialCache.delete(instanceId);
		return null;
	}
 ...
 * @param {string} credentialData.api_key - Airtable Personal Access Token
...
export function setCachedCredential(instanceId, credentialData) {
	...
	airtableCredentialCache.set(instanceId, cacheEntry);
...
export function getCacheStatistics() {
	...
	// Calculate memory usage without exposing sensitive data
	const sanitizedEntries = Array.from(airtableCredentialCache.entries()).map(([key, value]) => [
		key,
		{
			...value,
			credential: '[REDACTED]' // Replace actual credential with placeholder
		}
	]);
	...
	return {
		...
		memory_usage_mb: (JSON.stringify(sanitizedEntries).length / 1024 / 1024).toFixed(2)
	};
}
...
export function updateCachedCredentialMetadata(instanceId, updates) {
	...
	airtableCredentialCache.set(instanceId, cached);
	return true;
}
...
export function cleanupInvalidCacheEntries(reason = 'cleanup') {
	...
	for (const [instanceId, cached] of airtableCredentialCache.entries()) {
