...
/**
 * Airtable API Key Validator that implements BaseValidator interface
 */
class AirtableAPIKeyValidator {
	constructor() {
		this.validator = new CredentialValidator();
		this.serviceName = 'airtable';
	...
	async validateFormat(credentials) {
		try {
			...
			return {
				...
				field: result.field || 'apiKey',
				service_info: this.getServiceInfo(credentials)
			};
		} catch (error) {
			return {
				valid: false,
 ...
 * @param {any} credentials - Credentials to validate
 * @returns {AirtableAPIKeyValidator} Validator instance
 */
function createAirtableValidator(credentials) {
	if (credentials && (credentials.apiKey || credentials.api_key)) {
		return new AirtableAPIKeyValidator();
	} else {
		throw new Error('Invalid Airtable credentials format - must provide apiKey or api_key');
	}
}
