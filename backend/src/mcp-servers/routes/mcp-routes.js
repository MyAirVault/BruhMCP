    ...
    mcpRouter.get('/:endpoint', async (req, res) => {
        try {
            ...
            if (error.message.includes('not supported')) {
                ...
                // Log error
                if (mcpId) {
                    logFileManager.writeLog(mcpId, 'error', `Generic endpoint failed: ${req.params.endpoint} - ${error.message}`, 'error', {
    ...
    mcpRouter.get('/:endpoint/*', async (req, res) => {
        try {
            ...
            if (error.message.includes('not supported') || error.message.includes('Invalid parameters')) {
                ...
                // Log error
                if (mcpId) {
                    logFileManager.writeLog(mcpId, 'error', `Parameterized endpoint failed: ${req.params.endpoint} - ${error.message}`, 'error', {
                        endpoint: req.params.endpoint,
