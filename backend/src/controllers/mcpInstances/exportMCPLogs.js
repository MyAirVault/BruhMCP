...
export async function exportMCPLogs(req, res) {
	try {
		...
		const logBasePath = path.join(__dirname, '..', '..', '..', 'logs', 'users', `user_${userId}`, `mcp_${id}`);
		...
		const filename = `logs_${id}_${timestamp}.${format}`;
		const exportPath = path.join(__dirname, '..', '..', '..', 'public', 'downloads', filename);

		// Create exports directory if it doesn't exist
		const exportsDir = path.dirname(exportPath);
		try {
			await stat(exportsDir);
		} catch {
			await import('fs/promises').then(fs => fs.mkdir(exportsDir, { recursive: true }));
		}
